...
function Fourier_linop(X::CartesianSpatialGeometry{T}, K::AbstractStructuredKSpaceSampling{T}; normalization::T=1/T(sqrt(prod(X.nsamples))), tol::T=T(1e-6)) where {T<:Real}
    o = origin(X; wrt_center=true)
    origin_phase_shift = exp.(im*sum(coord(K).*reshape([o...], 1, 1, 3); dims=3)[:, :, 1])
    return NFFTtype2LinOp{T}(X, K, origin_phase_shift, normalization, tol)
end


## General type-2 NFFT linear operator

struct StructuredNFFTtype2LinOp{T<:Real}<:AbstractRigidMotionPerturbedFFT{T,AbstractCartesianSpatialGeometry{T},AbstractStructuredKSpaceSampling{T},3,2}
